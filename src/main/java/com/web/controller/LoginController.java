...
public class LoginController {
    ...
    *  페이스북 인증에만 쓸 수 있어서 구글, 카카오는 추가 코드 필요
    ...
    //이 예제에서는 두번째 방법 사용
    //HandlerMethodArgumentResolver 인터페이스는 전략 패턴의 일종
    //컨트롤러 메서드에서 특정 조건에 해당하는 파라미터가 잇으면 생성한 로직을 처리한 후 해당 파라미터에 바인딩 해주는 전략
    //*전략 패턴 : 특정 전략을 인터페이스로 만들고 이를 여러 전략 객체로 구현. 현재 클래스 레벨에서 전략 인터페이스를 의존하도록 설정
}
